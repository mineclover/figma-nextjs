...
export interface SectionSelectSvgMainResponseHandler extends EventHandler {
	...
	handler: (lazyNodes: LazyNodeData[]) => void;
}

export interface GenerateSvgFromLazyNodesUiRequestHandler extends EventHandler {
	name: "GENERATE_SVG_FROM_LAZY_NODES_UI_REQUEST";
	handler: (lazyNodes: LazyNodeData[], filter: FilterType) => void;
}

export interface GenerateSvgFromLazyNodesMainResponseHandler
	extends EventHandler {
	name: "GENERATE_SVG_FROM_LAZY_NODES_MAIN_RESPONSE";
	handler: (svgResult: SVGResult) => void;
}

export interface GetNodeInfoUiRequestHandler extends EventHandler {
	name: "GET_NODE_INFO_UI_REQUEST";
	handler: (nodeId: string, pageId: string) => void;
}

export interface GetNodeInfoMainResponseHandler extends EventHandler {
	name: "GET_NODE_INFO_MAIN_RESPONSE";
	handler: (nodeInfo: {
		id: string;
		name: string;
		type: string;

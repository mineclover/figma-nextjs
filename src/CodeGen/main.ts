...
export default function () {
  if (['dev', 'figma'].includes(figma.editorType)) {
    ...
    on<MessageHandler>('POST_MESSAGE', (text: string) => {
    ...
    // Variables
    // #region
    on<VariableGetRequestHandler>('VARIABLE_GET_REQUEST', async () => {
      const remoteParent = {
      ...
      /** 전체 스타일 이름
       * 키 이름을 변수 이름으로 잡고 , 벨류 리스트 즉 이름에 할당된 변수들로 추론한다
       */
       ...
       *
       * 블렌드 모드까지 쓰려면 scss로 써야하나? 어떻게?
       * 굳이도 포함
       * 블렌드랑 그라디언트를 분리하는게 좋을 것 같고 그 이유는 스펙 확인하려고

...
import { SvgSymbolHandler, CloseHandler, ScanHandler, MessageHandler } from './types';
...
export default function () {
  ...
  <defs>
    ...
    figma.on('selectionchange', async function () {
      const current = figma.currentPage.selection;
      if (current.length === 1) {
        const { completed, duplicate, unsupportedKeys, id } = await toSvg(current);

        const codeSnippet = '<SvgUse src="/$1/asset.svg#$0" className="$1" alt="$0" />';
        const $0 = id[0];
        const $1 = 'icon';
